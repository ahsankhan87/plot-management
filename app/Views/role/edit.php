...
<div class="container mx-auto p-4">
    ...
    <form method="post" class="space-y-4">
        <div>
            ...
            <input type="text" name="name" class="border rounded w-full px-3 py-2" value="<?= old('name', $role['name']) ?>">
            <?php if (isset($errors['name'])): ?><div class="text-red-500 text-sm"><?= $errors['name'] ?></div><?php endif; ?>
        </div>
        <div>
            <label class="block mb-1">Description</label>
            <input type="text" name="description" class="border rounded w-full px-3 py-2" value="<?= old('description', $role['description']) ?>">
